...
{
    /// <summary>
    ///     A <see cref="StringObjectRenderer{TToken}" /> for rendering <see cref="InterpolationToken" />s with support for
    ///     string formatting
    /// </summary>
    public class FormattedInterpolationTokenRenderer : StringObjectRenderer<InterpolationToken>
    {
        ...
        protected override void Write(StringRender renderer, InterpolationToken obj, Context context)
        {
            ...
            if (!context.RenderSettings.SkipHtmlEncoding && obj.EscapeResult && value != null) formattedValue = WebUtility.HtmlEncode(formattedValue);

            if (obj.Indent > 0) renderer.Write(' ', obj.Indent);
        ...
        protected override async Task WriteAsync(StringRender renderer, InterpolationToken obj, Context context)
        {
            ...
            if (!context.RenderSettings.SkipHtmlEncoding && obj.EscapeResult && value != null) value = WebUtility.HtmlEncode(value.ToString());
